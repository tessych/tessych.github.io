...
const copy: Record<Locale, Copy> = {
  ru: {
    ...
    focus: [{ title: 'Сначала понять', body: 'Сначала разбираюсь в задаче и данных. Архитектура появляется из контекста, а не из привычки.' }, { title: 'Оставить рабочим', body: 'Критичную логику покрываю тестами, чтобы новые изменения не ломали уже сделанное.' }, { title: 'Довести до запуска', body: 'Собираю окружение и контейнеры так, чтобы сервис работал не только на локалке.' }],
  ...
  en: {
    ...
    focus: [{ title: 'Understand first', body: 'I start with the problem and the data. Architecture should come from context, not habit.' }, { title: 'Keep it working', body: 'I test critical logic so new changes do not break the work already in place.' }, { title: 'Take it to launch', body: 'I set up the environment and containers so a service works beyond local development.' }],
  ...
  ja: {
    ...
    focus: [{ title: 'まず理解する', body: '課題とデータを最初に見ます。アーキテクチャは習慣ではなく、文脈から生まれるものです。' }, { title: '動く状態を守る', body: '重要なロジックをテストし、新しい変更で既存の動作を壊さないようにします。' }, { title: '起動まで届ける', body: 'ローカル環境だけでなくサービスが動くように、環境とコンテナを整えます。' }],
